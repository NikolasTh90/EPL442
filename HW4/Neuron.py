...
class RBFNeuron(Neuron):
    ...
    def __init__(self, center, sigma, isOutput=False):
        super().__init__(isBias=False, isOutput=isOutput)
        self.center = center
        self.sigma = sigma
        self.spread = self.calculateSpread()
        self.inputs = []

    # Calculate the spread of the RBF neuron based on the sigma value
    ...
    def gaussian_rbf(self, x):
        return np.exp(-self.spread * np.linalg.norm(x - self.center) ** 2)

    # Override getOutput() to return the output of the RBF neuron using the gaussian function
    def getOutput(self):
        return self.gaussian_rbf(self.input)
    
    # set Inputs for RBF Neuron
